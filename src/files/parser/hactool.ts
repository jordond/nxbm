...
import { ensureDir, move, pathExists, rename } from "fs-extra";
...
const log = create("hactool");

export async function ensureHactool(dataDirectory: string): Promise<boolean> {
...
async function compileHactool(path: string) {
  const makeLog = create("hactool:make");
