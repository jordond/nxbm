...
class AESCTR:
    ...
    def __init__(self, key, ctr):
        ...
        self.ctr = int(ctr.encode('hex'), 0x10)
    ...
    def encrypt(self, data, ctr=None):
        ...
        if ctr is None:
            ...
            ctr = int(ctr.encode('hex'), 16)
        out = ''
        ln = len(data)
        while ln:
            xorpad = self.aes.encrypt_block_ecb(('%032X' % ctr).decode('hex'))
            l = min(0x10, ln)
...
class AESXTS:
    ...
    def encrypt(self, data, sector=None):
        ...
        while data:
            ...
            out += self.encrypt_sector(data[:0x200], tweak)
            data = data[0x200:]
            sector += 1
        return out
    ...
    def encrypt_sector(self, data, tweak):
        if len(data) % self.block_size:
            raise ValueError('Data is not aligned to block size!')
        out = ''
        tweak = self.K2.encrypt(('%032X' % tweak).decode('hex'))
        while data:
            out += sxor(tweak, self.K1.encrypt_block_ecb(sxor(data[:0x10], tweak)))
            _t = int(tweak[::-1].encode('hex'), 16)
            _t <<= 1
            if _t & (1 << 128):
                _t ^= ((1 << 128) | (0x87))
            tweak = ('%032X' % _t).decode('hex')[::-1]
            data = data[0x10:]
        return out
    ...
    def decrypt(self, data, sector=None):
        ...
        while data:
            tweak = self.get_tweak(sector)
            out += self.decrypt_sector(data[:0x200], tweak)
            data = data[0x200:]
            sector += 1
        return out
    ...
    def get_tweak(self, sector=None):
        ...
        for i in xrange(self.block_size):
            tweak |= (sector & 0xFF) << ((self.block_size - i - 1) * 8)
            sector >>= 8
        return tweak
...
class AESECB:
    ...
    def __init__(self, key):
        ...
        for i in xrange(1, self.num_rounds+1):
    ...
    def encrypt_block_ecb(self, block):
        ...
        for rnd in xrange(1, self.num_rounds + 1):
            for i in xrange(len(words)):
    ...
    def decrypt_block_ecb(self, block):
        ...
        for rnd in xrange(self.num_rounds, 0, -1):
            for i in xrange(len(words)):
                words[i] ^= self.keys[rnd][i]
            if rnd != self.num_rounds:
